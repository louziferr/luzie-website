"use client";
export default function Programmierung() {
  return (
    <>
      <div className="pt-[40px] md:pt-[90px] bg-computer p-4 flex items-center justify-center flex-col">
        <div className="p-4 max-w-screen-xl">
          <div>
            <h1 className="text-left py-8 text-3xl md:text-6xl text-white">
              Über mich
            </h1>
            <div className="flex max-w-screen-xl">
              <div className="relative grid grid-cols-1 md:grid-cols-2">
                <p className="md:text-justify leading-relaxed order-2 md:order-1 text-white">
                  Im Rahmen meines Bachelorstudiums der Computerlinguistik habe
                  ich das Programmieren vom Anfang an mit hohem
                  wissenschaftlichem Bezug gelernt. Besonders Machine Learning
                  und AI im Kontext von Natural Language Processing waren
                  während des Bachelors wichtige Thematiken. Neben dem Studium
                  habe ich bereits damals als Tutorin an der Uni gearbeitet und
                  auch Kurse zu MINT-Themen für Schulen konzipiert und
                  durchgeführt.
                  <br />
                  2020 begann ich das Masterstudium in Audiokommunikation und
                  -technologie. Durch diverse Nebenjobs während des Studiums
                  lernte ich, das theoretische Wissen der Informatik anzuwenden
                  und effizienten Code zu schreiben. Nach einem längeren
                  Auslandsaufenthalt in Dänemark, habe ich begonnen, im Rahmen
                  von Programmen zur Frauenförderung Kurse zur Programmierung zu
                  geben. An Schulen, Unis und Ausbildungsinstituten habe ich
                  seitdem diversen Zielgruppen die Informatik näher gebracht.
                  <br />
                  <br />
                  Seit 2024 bin ich freiberuflich im Bereich der Informatik
                  tätig und schätze die Kombination aus technischer Arbeit und
                  Wissensvermittlung.
                  <br />
                  <br />
                </p>
                <div className="pt-0 p-4 flex items-start justify-center order-1 md:order-2">
                  <img
                    src="cool_me.jpeg"
                    alt="ME"
                    className="h-[250px] p-4 md:h-[550px] animate-delay-100 animate-fade shadow-md shadow-black bg-white"
                  />
                </div>
              </div>
            </div>
          </div>
        </div>

        <h1 className="md:pt-8 text-xl md:text-4xl text-white underline underline-offset-4">
          Lebenslauf
        </h1>

        <div className="md:p-8 flex items-start justify-start">
          <div className="p-2 md:p-8 leading-relaxed max-w-screen-xl">
            <ul className="font-saira">
              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-200 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[12px] h-[12px]"></div>
                  <span className="absolute inline-flex w-[12px] h-[12px] animate-ping rounded-full bg-sky-200 opacity-75"></span>
                </div>
                <p className="text-white">
                  Heute - freiberufliche Informatikerin
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  seit 2024 - Dozentin für Informatik bei IBB Institut für
                  berufliche Bildung AG
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-300 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  <b>
                    2024 - Master of Science: Audiokommunikation und
                    -technologie
                  </b>
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  2024 - Dozentin für Webentwicklung bei Devhaus Leipzig GmbH
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  seit 2023 - Gastdozentin für Python an der Uni Bremen
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  seit 2022 - Gastdozentin für Python an der TU Berlin
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  seit 2022 - Leitung der Robotik AG an der BIP
                  Kreativitätsschule Leipzig
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  2021 - 2022 - studentische Hilfskraft an der Fachhochschule
                  Kiel GmbH
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-300 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  <b>2016 - 2020 - Bachelor of Science: Computerlinguistik</b>
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  2018 - 2020 - MINT-Tutorin an der Uni Potsdam
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  2017 - 2019 - studentische Hilfskraft an der Uni Potsdam
                </p>
              </li>

              <li className="p-1 flex items-center">
                <div className="relative flex">
                  <div className="bg-sky-500 text-xl md:text-3xl font-alfa mr-3 rounded-full w-[10px] h-[10px]"></div>
                </div>
                <p className="text-white">
                  seit 2008 - gelegentliche Tätigkeit als Schauspielerin
                </p>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </>
  );
}
